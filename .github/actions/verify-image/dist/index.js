 ...
 */
function emitBuildArgs(args, format) {
    const entries = [...args];
    switch (format) {
        case "env":
        case "github-env":
        ...
        case "bake":
            return entries.map(([key, value]) => `--set *.args.${key}=${value}`);
        default: {
...
class BuildArgsParseError extends Error {
    path;
    line;
    constructor(path, line, reason) {
 ...
 */
function parseBuildArgs(content, path) {
    ...
    lines.forEach((rawLine, index) => {
        ...
        if (!match) {
            throw new BuildArgsParseError(path, lineNumber, `does not match KEY=VALUE (no spaces, quotes, or inline comments): ${JSON.stringify(rawLine)}`);
        }
        const [, key, value] = match;
        if (FORBIDDEN_VALUE_CHARS.test(value)) {
            throw new BuildArgsParseError(path, lineNumber, `value must not contain '#', "'", or '"': ${JSON.stringify(rawLine)}`);
        }
        if (result.has(key)) {
 ...
 */
function resolveMounts(mounts, manifestDir) {
    if (!mounts) {
        return undefined;
    }
    return mounts.map((m) => ({
        host: external_node_path_default().isAbsolute(m.host) ? m.host : external_node_path_default().resolve(manifestDir, m.host),
...
function fail(index, kind, label, detail) {
 ...
 */
function truncateOutput(output, maxLines = 5) {
    const lines = output
        .replace(/\r/g, "")
        .split("\n")
        .filter((line) => line.length > 0);
 ...
 * PASS 2 task brief).
 */
async function executeCmdCheck(check, index, ctx) {
 ...
 */
async function inspectImageConfig(cli, ref) {
    ...
    const env = Array.isArray(config.Env)
        ? config.Env.filter((v) => typeof v === "string")
        : [];
    const exposedPorts = typeof config.ExposedPorts === "object" && config.ExposedPorts !== null
        ? config.ExposedPorts
        : {};
 ...
 */
async function executeConfigUserCheck(check, index, ctx) {
 ...
 */
async function executeEnvCheck(check, index, ctx) {
    const config = await inspectImageConfig(ctx.cli, ctx.image);
...
async function executeExposedPortCheck(check, index, ctx) {
...
async function executeFileCheck(check, index, ctx) {
...
function hostPortToUrl(mapping, path) {
 ...
 */
async function executeHttpCheck(check, index, ctx) {
...
function isRuntimeInjected(normalized) {
    if (RUNTIME_INJECTED_FILES.has(normalized)) {
        return true;
    }
    return RUNTIME_INJECTED_DIRS.some((dir) => normalized === dir || normalized.startsWith(`${dir}/`));
}
...
function parseTarEntries(buffer) {
    ...
    while (offset + BLOCK_SIZE <= buffer.length) {
        ...
        if (typeflag === "L") {
            ...
            const name = pendingLongName ?? (prefixField.length > 0 ? `${prefixField}/${nameField}` : nameField);
 ...
 * SIGPIPE hazard.
 */
function tarContainsPath(entries, wantPath) {
 ...
 */
async function listImageFiles(cli, image) {
 ...
 */
async function executeImageFileCheck(check, index, ctx) {
    const label = `present: ${check.paths.join(" ")}`;
    let entries;
    try {
 ...
 */
async function executeShCheck(check, index, ctx) {
    const label = check.desc;
 ...
 */
async function executeUserCheck(check, index, ctx) {
    const label = check.name !== undefined
 ...
 */
async function executeVersionCheck(check, index, ctx) {
 ...
 */
async function executeWorkdirCheck(check, index, ctx) {
 ...
 */
async function executeWritableCheck(check, index, ctx) {
...
function executeCheck(check, index, ctx) {
 ...
 * in time.
 */
function spawnExec(args, opts) {
    return new Promise((resolve, reject) => {
        const child = (0,external_node_child_process_namespaceObject.spawn)("docker", [...args], { stdio: ["ignore", "pipe", "pipe"] });
        const chunks = [];
        let timedOut = false;
 ...
 */
function spawnExecBinary(args, opts) {
    return new Promise((resolve, reject) => {
        const child = (0,external_node_child_process_namespaceObject.spawn)("docker", [...args], { stdio: ["ignore", "pipe", "pipe"] });
        const stdoutChunks = [];
        const stderrChunks = [];
 ...
 */
function createDockerCli(exec = spawnExec, execBinary = spawnExecBinary) {
    return {
        async inspect(ref) {
            const result = await exec(["inspect", ref], { timeoutMs: DEFAULT_TIMEOUT_MS });
            if (result.exitCode !== 0) {
                throw new Error(`docker inspect ${ref} failed (exit ${String(result.exitCode)}): ${result.output}`);
            }
        ...
        async create(options) {
            ...
            if (result.exitCode !== 0) {
                throw new Error(`docker create ${options.image} failed (exit ${String(result.exitCode)}): ${result.output}`);
            }
        ...
        async export(containerId) {
            ...
            if (result.exitCode !== 0) {
                throw new Error(`docker export ${containerId} failed (exit ${String(result.exitCode)}): ${result.stderr}`);
            }
        ...
        async pull(ref) {
            ...
            if (result.exitCode !== 0) {
                throw new Error(`docker pull ${ref} failed (exit ${String(result.exitCode)}): ${result.output}`);
            }
        },
        async port(containerId, containerPort, protocol = "tcp") {
            ...
            if (result.exitCode !== 0) {
                throw new Error(`docker port ${containerId} ${spec} failed (exit ${String(result.exitCode)}): ${result.output}`);
            }
            return result.output.trim();
        },
        async start(containerId) {
            ...
            if (result.exitCode !== 0) {
                throw new Error(`docker start ${containerId} failed (exit ${String(result.exitCode)}): ${result.output}`);
            }
        },
        async rm(containerId, options) {
            ...
            const result = await exec(args, { timeoutMs: DEFAULT_TIMEOUT_MS });
            if (result.exitCode !== 0) {
                throw new Error(`docker rm ${containerId} failed (exit ${String(result.exitCode)}): ${result.output}`);
            }
        },
    };
}
...
const ENV_VARIANT_FIELDS = ["equals", "contains", "absent"];
...
function validateAsIdentity(value, location) {
    ...
    schema_fail(location, `"as" must be "default", "root", or a non-negative integer uid, got ${JSON.stringify(value)}`);
}
function validateStringArray(value, field, location) {
    if (!Array.isArray(value) || value.length === 0 || !value.every((v) => typeof v === "string")) {
        schema_fail(location, `"${field}" must be a non-empty array of strings`);
    }
}
/** Validates a single check object against its kind's field spec. Throws {@link ManifestError} on any violation. */
function validateCheck(raw, location) {
    if (!isPlainObject(raw)) {
        schema_fail(location, "check must be an object");
    }
    const kind = raw.kind;
    if (typeof kind !== "string" || !CHECK_KINDS.includes(kind)) {
        schema_fail(location, `unknown check kind ${JSON.stringify(kind)} (valid kinds: ${CHECK_KINDS.join(", ")})`);
    }
    const spec = CHECK_FIELD_SPECS[kind];
    const allowedFields = ["kind", ...spec.required, ...spec.optional];
    checkUnknownFields(raw, allowedFields, location);
    if (kind === "env") {
        if (typeof raw.name !== "string" || raw.name.length === 0) {
            schema_fail(location, `"name" is required and must be a non-empty string`);
        }
        const present = ENV_VARIANT_FIELDS.filter((f) => raw[f] !== undefined);
        if (present.length !== 1) {
            schema_fail(location, `env check must specify exactly one of ${ENV_VARIANT_FIELDS.join(", ")}, got: ${present.length === 0 ? "none" : present.join(", ")}`);
        }
        if (present[0] === "absent" && raw.absent !== true) {
            schema_fail(location, `"absent" must be literal true`);
        }
        return raw;
    }
    for (const field of spec.required) {
        if (raw[field] === undefined) {
            schema_fail(location, `missing required field "${field}" for kind "${kind}"`);
        }
    }
    switch (kind) {
        case "user":
            if (typeof raw.uid !== "number" && typeof raw.uid !== "string") {
                schema_fail(location, `"uid" must be a number or string`);
            }
            break;
        case "configUser":
        case "workdir":
            if (typeof raw.value !== "string") {
                schema_fail(location, `"value" must be a string`);
            }
            break;
        case "cmd":
        ...
        case "version":
            if (typeof raw.run !== "string" || raw.run.length === 0) {
                schema_fail(location, `"run" must be a non-empty string`);
            }
            if (typeof raw.contains !== "string") {
                schema_fail(location, `"contains" must be a string`);
            }
            break;
        case "writable":
        case "file":
        ...
        case "sh":
            if (typeof raw.desc !== "string" || raw.desc.length === 0) {
                schema_fail(location, `"desc" must be a non-empty string`);
            }
            if (typeof raw.run !== "string" || raw.run.length === 0) {
                schema_fail(location, `"run" must be a non-empty string`);
            }
        ...
        case "http":
            ...
            if (typeof raw.containerPort !== "number") {
                schema_fail(location, `"containerPort" must be a number`);
            }
            if (typeof raw.path !== "string" || raw.path.length === 0) {
                schema_fail(location, `"path" must be a non-empty string`);
            }
            if (typeof raw.expectStatus !== "number") {
                schema_fail(location, `"expectStatus" must be a number`);
            }
            break;
    }
    if ("as" in raw) {
...
function validateChecksArray(raw, location) {
    if (!Array.isArray(raw)) {
        schema_fail(location, `"checks" must be an array`);
    }
    if (raw.length === 0) {
        schema_fail(location, `"checks" must not be empty`);
    }
...
function validateManifest(raw) {
    ...
    if (raw.buildargs !== undefined && typeof raw.buildargs !== "string") {
        schema_fail("buildargs", `"buildargs" must be a string`);
    }
    if (raw.registry !== undefined && typeof raw.registry !== "string") {
        schema_fail("registry", `"registry" must be a string`);
    }
    let defaults;
    if (raw.defaults !== undefined) {
        if (!isPlainObject(raw.defaults)) {
            schema_fail("defaults", `"defaults" must be an object`);
        }
        checkUnknownFields(raw.defaults, ["checks"], "defaults");
        if (raw.defaults.checks === undefined) {
            schema_fail("defaults", `"defaults" must have a "checks" field`);
        }
        defaults = { checks: validateChecksArray(raw.defaults.checks, "defaults.checks") };
    }
    if (raw.targets === undefined) {
        schema_fail("targets", `manifest must have a "targets" field`);
    }
    if (!Array.isArray(raw.targets) || raw.targets.length === 0) {
        schema_fail("targets", `"targets" must be a non-empty array`);
    }
    const targets = raw.targets.map((entry, idx) => {
        const location = `targets[${idx}]`;
        if (!isPlainObject(entry)) {
            schema_fail(location, "target entry must be an object");
        }
        ...
        if (entry.checks === undefined) {
            schema_fail(location, `target entry must have a "checks" field`);
        }
    ...
    return {
        ...
        targets,
        ...(raw.buildargs !== undefined && { buildargs: raw.buildargs }),
...
function globMatch(pattern, targetName) {
...
function resolveTargets(manifest, targetNames) {
    ...
    manifest.targets.forEach((entry, idx) => {
        ...
        if (matched.length === 0) {
            throw new ManifestError(`match pattern "${entry.match}" matched none of the ${targetNames.length} known bake targets`, `targets[${idx}].match`);
        }
 ...
 */
function substituteString(input, vars, location) {
    let result = "";
    let i = 0;
...
function substituteValue(value, vars, location) {
 ...
 */
function substituteManifest(manifest, vars) {
 ...
 */
function formatConsoleReport(results, options = {}) {
...
function printConsoleReport(results, stream = process.stdout) {
    const color = "isTTY" in stream && stream.isTTY === true;
 ...
 */
function parseBakePrint(json) {
    let raw;
    try {
        raw = JSON.parse(json);
    }
    catch (error) {
        throw new BakeError(`invalid bake --print JSON: ${error instanceof Error ? error.message : String(error)}`);
    }
 ...
 */
function spawnBakeExec(args, opts) {
    return new Promise((resolve, reject) => {
...
const bake_DEFAULT_TIMEOUT_MS = 120_000;
/**
 ...
 * failure.
 */
async function runBakePrint(exec = spawnBakeExec, options = {}) {
    const args = ["buildx", "bake", "--print", ...(options.args ?? [])];
    const timeoutMs = options.timeoutMs ?? bake_DEFAULT_TIMEOUT_MS;
    ...
    if (result.exitCode !== 0) {
        throw new BakeError(`docker ${args.join(" ")} failed (exit ${String(result.exitCode)}): ${result.stderr || result.stdout}`);
    }
 ...
 */
function deriveRepoFromTag(tag) {
    return tag.replace(/:[^/]*$/, "");
}
function extractArchitecture(raw) {
    if (!Array.isArray(raw) || raw.length === 0) {
        return "unknown";
    }
    const entry = raw[0];
 ...
 */
async function resolveLocalTarget(cli, target) {
    ...
    try {
        ...
        throw new ResolveError(`image "${ref}" for target "${target.name}" is not loaded locally — build it first ` +
            `(e.g. \`docker buildx bake --load ${target.name}\`): ${error instanceof Error ? error.message : String(error)}`);
    }
 ...
 */
async function resolveDigestTarget(cli, target, digests) {
    ...
    try {
        ...
        throw new ResolveError(`docker pull ${ref} failed for target "${target.name}": ${error instanceof Error ? error.message : String(error)}`);
    }
    let raw;
    try {
        raw = await cli.inspect(ref);
...
function parseArgv(argv) {
    ...
    const args = { targets: [], noColor: false };
    let i = 0;
    while (i < rest.length) {
        const flag = rest[i];
        if (flag === undefined) {
...
async function loadManifest(args) {
    const manifestPath = external_node_path_default().resolve(args.manifest ?? DEFAULT_MANIFEST_PATH);
    const manifestDir = external_node_path_default().dirname(manifestPath);
    let manifestContent;
    try {
        manifestContent = await (0,promises_namespaceObject.readFile)(manifestPath, "utf8");
    }
    catch (error) {
    ...
    const buildargsPath = external_node_path_default().resolve(args.buildargs ??
        (manifest.buildargs !== undefined
            ? external_node_path_default().resolve(manifestDir, manifest.buildargs)
            : DEFAULT_BUILDARGS_PATH));
    let buildargsContent;
    try {
        buildargsContent = await (0,promises_namespaceObject.readFile)(buildargsPath, "utf8");
    }
    catch (error) {
...
async function loadBakeTargets(args, registry, timeoutMs, bakeExec) {
    if (args.bakePrint !== undefined) {
        const bakePrintPath = external_node_path_default().resolve(args.bakePrint);
        let content;
        try {
            content = await (0,promises_namespaceObject.readFile)(bakePrintPath, "utf8");
        }
        catch (error) {
 ...
 */
async function loadDigests(digestsFlag) {
    const digestsPath = external_node_path_default().resolve(digestsFlag);
    let raw;
    try {
        raw = JSON.parse(await (0,promises_namespaceObject.readFile)(digestsPath, "utf8"));
    }
    catch (error) {
    ...
    const digests = {};
    for (const [target, digest] of Object.entries(raw)) {
        if (typeof digest !== "string") {
            throw new UsageError(`--digests file ${digestsPath}: digest for "${target}" must be a string, got ${typeof digest}`);
        }
 ...
 */
function timeoutOverrideExec(timeoutMs) {
    if (timeoutMs === undefined) {
        return { exec: spawnExec, execBinary: spawnExecBinary };
    }
    const resolve = (requested) => requested === DOCKER_CLI_DEFAULT_TIMEOUT_MS ? timeoutMs : requested;
    return {
...
function targetHeader(resolved) {
 ...
 */
async function verifyTarget(target, ctx) {
    ...
    const checks = resolvedChecks.get(target.name) ?? [];
    const results = [];
    let hardError;
...
async function runValidateCommand(args) {
    ...
    if (args.bakePrint === undefined) {
        process.stdout.write("manifest OK (schema + \\${VAR} substitution only — pass --bake-print to also validate " +
...
async function runBuildArgsCommand(args) {
    if (args.format !== "env" && args.format !== "bake" && args.format !== "github-env") {
        throw new UsageError(`--format must be one of env, bake, github-env`);
    }
    const buildargsPath = external_node_path_default().resolve(args.buildargs ?? DEFAULT_BUILDARGS_PATH);
    const content = await (0,promises_namespaceObject.readFile)(buildargsPath, "utf8");
    const vars = parseBuildArgs(content, buildargsPath);
    for (const line of emitBuildArgs(vars, args.format)) {
...
async function runRunCommand(args, deps) {
    ...
    let bakeTargets;
    try {
        bakeTargets = await loadBakeTargets(args, loaded.registry, timeoutMs, deps.bakeExec ?? spawnBakeExec);
    }
    catch (error) {
    ...
    if (args.targets.length > 0) {
        ...
        if (selectedTargets.length === 0) {
            throw new UsageError(`--target matched none of the ${String(bakeTargets.length)} known bake targets (${bakeTargets.map((t) => t.name).join(", ")})`);
        }
    }
    ...
    const outcomes = await runPool(selectedTargets, jobs, (target) => verifyTarget(target, {
        cli,
        digests,
        resolvedChecks,
